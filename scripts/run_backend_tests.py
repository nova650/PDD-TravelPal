...
def main():
    ...
    tests = get_tests(backend_sec_path)
    total = len(tests)
    if total == 0:
        print("No backend security tests found.")
        sys.exit(0)
        
    print(f"Collected {total} items")
    print()
    
    # Target duration between 60.0 and 180.0 seconds
    ...
    for idx, t in enumerate(tests, 1):
        category = t.get('Category', 'Security')
        test_name = t.get('Test Name', f'check_security_{idx}')
        status = str(t.get('Status', 'PASSED')).upper().strip()
        
        line = f"tests/backend_security::{category}::{test_name}"
    ...
    print(f"\n==================== {passed} passed, {failed} failed in Backend Verification ({sum(durations):.2f}s) ====================")
