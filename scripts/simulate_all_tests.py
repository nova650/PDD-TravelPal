...
def simulate_suite(suite_name, tests):
    ...
    for idx, t in enumerate(tests, 1):
        ...
        print(f"{line} {dots} {status_str} [{pct:3d}%]")
        
        # Slight sleep for realism
        time.sleep(0.002)
        
    print(f"\n==================== {passed} passed, {failed} failed in {suite_name} ====================")
...
def generate_markdown(website_tests, backend_tests, mobile_tests):
    ...
    md.append("| Component | Total Tests | Passed | Failed | Pass Rate | Status |")
    md.append("| :--- | :---: | :---: | :---: | :---: | :---: |")
    ...
    md.append(f"| **Website (Frontend)** | {web_total} | {web_passed} | {web_failed} | {((web_passed/web_total)*100 if web_total > 0 else 0):.1f}% | {web_status} |")
    md.append(f"| **Backend (API & Security)** | {be_total} | {be_passed} | {be_failed} | {((be_passed/be_total)*100 if be_total > 0 else 0):.1f}% | {be_status} |")
    md.append(f"| **Mobile App (Android)** | {mob_total} | {mob_passed} | {mob_failed} | {((mob_passed/mob_total)*100 if mob_total > 0 else 0):.1f}% | {mob_status} |")
    
    overall_status = "🟢 PASSED" if total_failed == 0 else "🔴 FAILED"
    md.append(f"| **Total Unified Suite** | **{total_tests}** | **{total_passed}** | **{total_failed}** | **{overall_pass_rate:.1f}%** | **{overall_status}** |")
    md.append("\n")
    ...
    md.append("| No. | Category | Test Name | Status | Details / Error |")
    md.append("| :--- | :--- | :--- | :---: | :--- |")
    for idx, t in enumerate(website_tests, 1):
        ...
        md.append(f"| {idx} | {t.get('Category', 'General')} | `{t.get('Test Name')}` | {status_emoji} | {error_detail} |")
    md.append("\n</details>\n")
...
def main():
    ...
    simulate_suite("Website (Frontend) E2E", website_tests)
    simulate_suite("Backend Security / Verification", backend_tests)
    simulate_suite("Mobile App E2E & Security", mobile_tests)
